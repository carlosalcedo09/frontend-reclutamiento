		...
		try {
			...
			if (error.response?.status === 400) {
				...
				setModalConfirmContent({
					...
					description: 'Hubo un problema al enviar tu postulación. Intenta nuevamente.',
					textbutton: 'Aceptar',
					onAction: () => setIsModalConfirmOpen(false),
				});
			}
					...
					>
						...
						<div className="flex-1 overflow-y-auto p-6 space-y-4">
							...
							<div className="flex gap-4">
								<div className="flex items-center gap-2 py-1">
									{selectedOffer.mode === 'Remoto' && (
										<House className="w-4 h-4 text-gray-500" />
									)}
									{selectedOffer.mode === 'Presencial' && (
										<Building className="w-4 h-4 text-gray-500" />
									)}
									{selectedOffer.mode === 'Híbrido' && (
										<Gpu className="w-4 h-4 text-gray-500" />
									)}
									<p className="text-gray-600 text-sm">{selectedOffer.mode}</p>
								</div>

								<div className="flex items-center gap-2">
									{selectedOffer.employment_type === 'Full-Time' && (
										<Clock2 className="w-4 h-4 text-gray-500" />
									)}
									{selectedOffer.employment_type === 'Part-Time' && (
								...
								<ul className="list-disc list-inside">
									{selectedOffer.benefits?.length > 0 ? (
										selectedOffer.benefits.map((ben) => (
											<li key={ben.id} className="text-gray-700 text-sm">
												{ben.description}
											</li>
										))
									) : (
										<li className="text-gray-700 text-sm">
											No hay beneficios definidas
										</li>
									)}
								</ul>
							</div>
						...
						<div className="flex items-center justify-center gap-2 py-5">
							<p className="text-gray-400 text-sm">
								Publicado el {selectedOffer.start_date} -{' '}
							</p>
							<p className="text-gray-400 text-sm">
								Finaliza el {selectedOffer.end_date}
							</p>
						</div>
